...
class ClusterEncoderTrainer:
  def __init__(
    ...
    self.best_valid_hit_rate = 0
    self.best_valid_loss = 1e9
    
    self.validation_loss = []
    self.validation_hit_rate = []
  ...
  def train_by_num_iter(self, num_iters):
    ...
    for i in tqdm(range(num_iters), dynamic_ncols=True):
      i += 1 # 1-indexed
    ...
    # save last checkpoint
    self.save_model(self.save_dir / f'last.pt')
  ...
  def _get_loss_pred_from_single_batch(self, batch):
    ...
    return loss, cluster_logits, loss_dict
  ...
  def validate(self):
    ...
    return total_validation_loss / total_num_tokens, 0, validation_metric_dict
